...
function getRiskGuidance(
    ...
    if (
        ...
        why.push(
            "A payment or registration-fee request was detected."
        );

        avoidItems.push(
            "Do not pay registration, processing, joining or security fees for the job."
        );
    }
    ...
    if (
        ...
        why.push(
            "The text mentions an OTP or OTP verification request."
        );

        avoidItems.push(
            "Never share an OTP with a recruiter or employer."
        );
    }
    ...
    if (
        ...
        why.push(
            "A UPI PIN-related request was detected."
        );

        avoidItems.push(
            "Never disclose your UPI PIN or banking authentication credentials."
        );
    }
    ...
    if (
        ...
        why.push(
            "Bank or financial account information is mentioned."
        );

        avoidItems.push(
            "Do not send bank, card, CVV or net-banking credentials through recruitment messages."
        );
    }
    ...
    if (
        ...
        why.push(
            "Aadhaar information is requested or mentioned."
        );

        avoidItems.push(
            "Do not send identity documents until the employer and the legitimate purpose are independently verified."
        );
    }
    ...
    if (
        ...
        why.push(
            "PAN information is mentioned."
        );

        avoidItems.push(
            "Avoid sending sensitive identity documents through an unverified recruiter."
        );
    }
    ...
    if (
        triggeredRules.some(
            (rule) =>
                rule.key === "noInterview"
        )
    ) {
        why.push(
            "The posting indicates that an interview is not required."
        );

        doItems.push(
            "Verify the employer's official hiring and interview process before accepting an offer."
        );
    }
    ...
    if (
        triggeredRules.some(
            (rule) =>
                rule.key === "fakeSelection"
        )
    ) {
        why.push(
            "The text contains immediate or guaranteed selection language."
        );

        avoidItems.push(
            "Do not treat an instant-selection message as proof that an offer is legitimate."
        );
    }
    ...
    if (
        ...
        why.push(
            "Urgency or deadline pressure is used to encourage immediate action."
        );

        avoidItems.push(
            "Do not make decisions simply because the message says the offer expires soon."
        );
    }
    ...
    if (
        ...
        why.push(
            "The salary or earning claim appears unusually high or guaranteed for the stated requirements."
        );

        doItems.push(
            "Compare the salary and role with the company's official careers page and other reliable job information."
        );
    }
    ...
    if (
        triggeredRules.some(
            (rule) =>
                rule.key === "personalEmail"
        )
    ) {
        why.push(
            "A free personal email provider is used for recruitment."
        );

        doItems.push(
            "Verify that the recruiter and company are connected through independently confirmed contact details."
        );
    }
    ...
    if (
        ...
        why.push(
            "WhatsApp is used as a recruitment contact channel."
        );

        doItems.push(
            "Confirm the recruiter through the company's official website instead of relying only on the provided contact."
        );
    }
    ...
    if (
        triggeredRules.some(
            (rule) =>
                rule.key === "noExperience"
        )
    ) {
        why.push(
            "The posting emphasizes that no experience is required."
        );
    }
    ...
    if (
        triggeredRules.some(
            (rule) =>
                rule.key === "suspiciousLanguage"
        )
    ) {
        why.push(
            "The posting contains strong promotional or guaranteed language."
        );

        avoidItems.push(
            "Do not rely on guaranteed-income or easy-money claims without independent verification."
        );
    }

    if (personalData.length >= 2) {
        why.push(
            `Multiple sensitive-data signals were detected: ${personalData.join(", ")}.`
        );

        avoidItems.push(
            "Do not submit multiple sensitive identity or financial details to an unverified recruiter."
        );
    }
    ...
    if (
        ...
        why.push(
            "Both a phone contact and a personal email address were detected."
        );
    }
    ...
    if (risk.level === "high") {
        doItems.unshift(
            "Stop before paying money or submitting sensitive information.",
            "Verify the employer independently using its official website and independently found contact information.",
            "If you already shared information or lost money, preserve messages, payment records and other evidence and contact the relevant bank/payment provider and official cybercrime channels."
        );

        avoidItems.unshift(
            "Do not continue the recruitment process until the employer and offer have been independently verified."
        );
    }

    if (risk.level === "suspicious") {
        doItems.unshift(
            "Pause and independently verify the company, recruiter and job opening before proceeding.",
            "Check whether the same vacancy exists on the company's official careers page."
        );

        avoidItems.unshift(
            "Do not pay money or share sensitive credentials while verification is incomplete."
        );
    }

    if (risk.level === "low") {
        doItems.unshift(
            "Verify the employer independently before accepting the offer.",
            "Prefer applying through the company's official careers page."
        );

        avoidItems.unshift(
            "Do not assume a low score guarantees that the job is genuine."
        );
    }
    ...
    if (!why.length) {
        why.push(
            `The calculated score is ${risk.score}/100 based on the predefined warning signals detected in the text.`
        );
    }

    if (!doItems.length) {
        doItems.push(
            "Verify the employer independently before sharing information or accepting the offer."
        );
    }

    if (!avoidItems.length) {
        avoidItems.push(
            "Do not share passwords, OTPs, UPI PINs or unnecessary financial credentials."
        );
    }

    return {
        why: [...new Set(why)],
        doItems: [...new Set(doItems)],
...
/* =========================================================
   NEW — RENDER INSTANT RISK PREVIEW
========================================================= */

function renderInstantRisk(analysis) {
    if (!instantRiskArea) {
        return;
    }
    ...
    if (instantRiskScore) {
        instantRiskScore.textContent =
            analysis.risk.score;
    }

    if (instantRiskPill) {
        instantRiskPill.textContent =
            info.pill;

        instantRiskPill.classList.remove(
            "risk-low",
            "risk-suspicious",
            "risk-high"
        );

        instantRiskPill.classList.add(
            `risk-${analysis.risk.level}`
        );
    }

    if (instantRiskTitle) {
        instantRiskTitle.textContent =
            info.title;
    }

    if (instantRiskReason) {
        instantRiskReason.textContent =
            info.explanation;
    }

    if (instantRiskTips) {
        instantRiskTips.innerHTML =
            guidance.doItems
                .slice(0, 3)
                .map(
                    (item) =>
                        `<div class="instant-tip">
                            <span>✓</span>
                            <span>${escapeHTML(
                                item
                            )}</span>
    ...
    if (instantRiskAvoid) {
        instantRiskAvoid.innerHTML =
            guidance.avoidItems
                .slice(0, 2)
                .map(
                    (item) =>
                        `<div class="instant-tip instant-tip-danger">
                            <span>!</span>
                            <span>${escapeHTML(
                                item
                            )}</span>
    ...
    instantRiskArea.classList.remove(
        "hidden"
    );
    ...
    instantRiskArea.classList.add(
        `risk-${analysis.risk.level}`
    );
}

/* =========================================================
   NEW — RENDER SCORE EXPLANATION
========================================================= */

function renderRiskGuidance(analysis) {
    const guidance =
        getRiskGuidance(
            analysis
        );
    ...
    if (riskWhySection) {
        riskWhySection.classList.remove(
            "hidden"
        );
    }

    if (riskWhyList) {
        riskWhyList.innerHTML =
            guidance.why
                .map(
                    (item) =>
    ...
    if (doList) {
        doList.innerHTML =
            guidance.doItems
                .map(
                    (item) =>
                        `<div class="guidance-item guidance-do">
                            <span class="guidance-icon">✓</span>
                            <span>${escapeHTML(
                                item
                            )}</span>
    ...
    if (avoidList) {
        avoidList.innerHTML =
            guidance.avoidItems
                .map(
                    (item) =>
                        `<div class="guidance-item guidance-avoid">
                            <span class="guidance-icon">!</span>
                            <span>${escapeHTML(
                                item
                            )}</span>
    ...
    if (riskActionTitle) {
        if (analysis.risk.level === "high") {
            riskActionTitle.textContent =
                "Immediate Safety Actions";
        } else if (
            analysis.risk.level === "suspicious"
        ) {
            riskActionTitle.textContent =
...
function displayResult(analysis) {
    ...
    /* NEW: show score near analyzer */
    renderInstantRisk(
        analysis
    );

    /* NEW: render score explanation + Do/Avoid */
    renderRiskGuidance(
        analysis
    );
...
async function handleAnalyze() {
    ...
    if (riskWhySection) {
        riskWhySection.classList.add(
            "hidden"
        );
    }

    if (doSection) {
        doSection.classList.add(
            "hidden"
        );
    }

    if (avoidSection) {
        avoidSection.classList.add(
            "hidden"
        );
    }
